import { isFailure, withResponse } from "@repo/tresponse";
...
> = {
  ...
  abort?: boolean;
  bg?: boolean;
  runIf?: ((results: TaskResults<T, I>) => boolean | Promise<boolean>)[];
  argMap?: (results: TaskResults<T, I>) => Parameters<F>[0];
};
...
export type TaskResults<T extends TaskMap, I> = { _init: I } & {
  [K in keyof T]: TaskFnResult<T[K]>;
};
...
export type TaskSchemaWithContracts<T extends TaskMap, I, O> = {
  ...
  _init?: I;
  _output: (results: TaskResults<T, I>) => O;
};
...
export const runSchema = withResponse(
  async <T extends TaskMap, I, O>(
    ...
    try {
      ...
      const results: Partial<TaskResults<T, I>> = { _init: initArgs } as any;
      const status: { [K in keyof typeof localSchema]: TaskState } = {} as any;
      ...
      let progress = true,
        safety = 0,
        maxIterations = Object.keys(localSchema).length * 2;
        ...
        for (const key of Object.keys(
          ...
          if (task.runIf?.length) {
            const runIfResults = await Promise.all(
              task.runIf.map((fn) => fn(results as TaskResults<T, I>))
            );
          ...
          const args = task.argMap?.(results as TaskResults<T, I>);
          ...
          if (isFailure(result)) {
            //TODO: log below only tmp
            console.log("task failed", key, result);
            status[key] = "failed";
      ...
      const pendingKeys = Object.keys(localSchema).filter(
        (k) => status[k as keyof typeof localSchema] === "pending"
      );
      ...
      //TODO: log below only tmp
      console.log("about to compute output");

      const output = schema._output(results as TaskResults<T, I>);

      //TODO: log below only tmp
      console.log("finishing with output", output);
